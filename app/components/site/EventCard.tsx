    ...
    >
      <Link to="/events/$id" params={{ id: e.id }} className="block">
        <div className="relative overflow-hidden">
          <img src={e.image} alt={e.title} loading="lazy" className={`w-full object-cover transition duration-700 group-hover:scale-105 ${large ? "aspect-[16/11]" : "aspect-[4/3]"}`} />
          ...
          <div className="absolute inset-x-4 bottom-4 text-white">
            ...
            <p className="mt-1 line-clamp-1 text-sm text-white/85">{e.tagline}</p>
            <div className="mt-3 flex items-center justify-between">
              <span className="flex items-center gap-1 text-xs text-white/80"><MapPin className="h-3 w-3" /> {e.venue}</span>
